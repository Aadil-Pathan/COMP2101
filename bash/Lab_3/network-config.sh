...
external_ip_address=$(curl -s icanhazip.com)
external_ip_address_name=$(getent hosts $external_ip_address | awk '{print $2}')


router_address=$(ip route | awk '/default/ {print $3}')
...
External Name   : $external_ip_address_name
